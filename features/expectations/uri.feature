...
Feature: URI
  ...
  Scenario: Matching HTTP message URI
    Given you expect "uri" to be "/animals?type=cats"
  ...
  Scenario: URI with the same parameters in different order
    Given you expect "uri" to be "/animals?type=cats&count=2"
  ...
  Scenario Outline: URI with parameter having multiple values
    Given you expect "uri" to be "/animals?type=cats&type=dogs"
